from model import Categoria, Estoque, Produtos, Fornecedor, Pessoa, Funcionario, Venda
...
# se não converter 'quantidade' para 'str' não consegue fazer a concatenação

    def alterarCategoria(self, categoriaAlterar, categoriaAlterada):
        x = DaoCategoria.ler()
        # no cat estamos fazendo um filtro para ver se existe a categoria que deseja alterar
        cat = list(filter(lambda x: x.categoria == categoriaAlterar, x))
        # se cat for maior que zero significa que eu posso alterar a categoria
        if len(cat) > 0:
            # cat1 é outro filtro para verificar se a categoria p/ qual deseja alterar já existe
            ...
            print('A categoria que desaja alterar não existe')
...
class ControllerVenda:
    ...
    def mostrarVenda(self, dataInicio, dataTermino):
        ...
        vendasSelecionadas = list(filter(lambda x: datetime.strptime(x.data, '%d/%m/%Y')
                                         >= dataInicio1 and datetime.strptime(x.data, '%d/%m/%Y')
                                         <= dataTermino1, vendas))
...
class ControllerFuncionario:
    def cadastrarFuncionario(self, clt, nome, telefone, cpf, email, endereco):
        ...
        if len(listaCpf) > 0:
            ...
            if len(cpf) == 11 and len(telefone) >= 10 and len(telefone) <=11:
                ...
                print('difite um cpf ou telefone válido')
    ...
    def removerFuncionario(selfself, nome):
        x = DaoFuncionario.ler()
